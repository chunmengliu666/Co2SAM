    ...
    with torch.no_grad():
        for iter, data in enumerate(val_dataloader):
            ...
            for i, label_id in enumerate(class_label[0]):
                ...
                logits_each = torch.tensor(detections.confidence)
                class_ids_each = detections.class_id
                class_ids_each = torch.tensor(class_ids_each.astype(np.float32))

                class_ids_each.nan_to_num(args.background_class)

                class_ids_each = class_ids_each.int()
                class_ids_each[class_ids_each < 0] = args.background_class
                transformed_boxes = model.apply_boxes_torch1(boxes, dino_images.shape[:2])

                prompts_boxes = torch.cat((prompts_boxes, transformed_boxes), dim=0)
                logits = torch.cat((logits, logits_each), dim=0)
                class_ids = torch.cat((class_ids, class_ids_each), dim=0)
            ...
            if len(transformed_boxes) > 0:
                prompts = (prompts_boxes.cuda().to(dtype=torch.float64),)
