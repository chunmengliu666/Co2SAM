...
class PascalVOCDataset(Dataset):
    def __init__(self, cfg, root_dir, transform=None, training=False, if_self_training=False):
        ...
        eval_list_img = open(os.path.join(root_dir, "ImageSets/SegmentationAug/test.txt")).read().splitlines()
        eval_list = []
        for eval_img_name in eval_list_img:
            eval_img_name = os.path.join(self.root_dir, eval_img_name[1:])

            eval_list.append(eval_img_name)

        if training:
            random.shuffle(train_list)
            image_ids = train_list
            # ./data/VOC2012/SegmentationClassAug/2008_008423.png
