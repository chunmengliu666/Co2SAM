...
def validate(fabric: L.Fabric, cfg: Box, args, model: Model, dino_model1, val_dataloader: DataLoader, name: str, epoch: int = 0):
    ...
    with torch.no_grad():
        for iter, data in enumerate(val_dataloader):
            ...
            # image = [1, 3, 1024, 1024], gt_mask[0] = [375, 500]
            # image_path[0] = './data/VOC2012/JPEGImages/2010_000087.jpg'


            dino_images = (image.squeeze(0).permute(1, 2, 0)*255).cpu().numpy().astype(np.uint8)
            # images = [1024, 1024, 3], numpy, minmax = [0, 255]
            ...
            if len(transformed_boxes) > 0:
                ...
                for label in class_label:
                    new_r_mask[label] = r_mask[label]
                new_r_mask[args.background_class] = r_mask[args.background_class]
            ...
            cam_wo_bg = new_r_mask[:args.background_class]
            cam_max = np.max(cam_wo_bg, (1,2), keepdims=True)
            cam_min = np.min(cam_wo_bg, (1,2), keepdims=True)
            norm_cam = (cam_wo_bg - cam_min - 1e-5) / (cam_max - cam_min + 1e-5)
            draw_single_heatmap(norm_cam, class_label[0], orig_img_ht, heatmap_folder, image_path[0][-15:-4])


            out = Image.fromarray(pseudo_label, mode='P')
            predict = torch.from_numpy(pseudo_label).unsqueeze(0).numpy()
            # predict = [1, 375, 500], minmax = [0, 15]

            out.putpalette(palette)
            # palette = imgviz.label_colormap(80)
            # out.putpalette(palette.flatten())
    ...
    # path = save_folder + 'scores/' + model_name + '/'
...
def draw_single_heatmap(norm_cam, gt_label, orig_img, save_path, img_name):
    ...
    for i, gt in enumerate(gt_label):
