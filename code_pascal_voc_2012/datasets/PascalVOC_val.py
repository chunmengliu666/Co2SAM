...
class PascalVOCDataset(Dataset):
    def __init__(self, cfg, root_dir, transform=None, training=False, if_self_training=False):
        ...
        segment_root = os.path.join(root_dir, "SegmentationClassAug")
        all_anns = [os.path.join(segment_root, f) for f in os.listdir(segment_root) if f.endswith('.png')]
        all_anns = sorted(all_anns)

        train_list_gt = open(os.path.join(root_dir, "ImageSets/SegmentationAug/train_aug.txt")).read().splitlines()
        train_list = []
        for train_gt_name in train_list_gt:
            train_gt_name = os.path.join(self.root_dir, train_gt_name.split(' ')[1][1:])
            train_list.append(train_gt_name)


        eval_list_gt = open(os.path.join(root_dir, "ImageSets/SegmentationAug/val.txt")).read().splitlines()
        eval_list = []
        for eval_gt_name in eval_list_gt:
            eval_gt_name = os.path.join(self.root_dir, eval_gt_name.split(' ')[1][1:])

            eval_gt_name = eval_gt_name.replace("SegmentationClassAug", "annotations_detectron2/val_bg")
    ...
    def __getitem__(self, idx):
        ...
        if self.if_self_training:
            image_path = anno_path.replace("SegmentationClassAug", "JPEGImages").replace(".png", ".jpg")
        else:
            image_path = anno_path.replace("annotations_detectron2/val_bg", "JPEGImages").replace(".png", ".jpg")
            # ./data/VOC2012/JPEGImages/2007_000515.jpg
        ...
        if self.if_self_training:
            ...
            categories = [item-1 for item in categories if item != 0]
        else:
            categories = [item for item in categories if item != 255]
            categories = [item for item in categories if item != 20]
